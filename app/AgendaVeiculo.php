...
class AgendaVeiculo extends Model {
	...
	public static function getReserva($id){
		return DB::select("SELECT agenda_veiculos.id, DATE_FORMAT(dia, '%d/%m/%Y') AS dia , TIME_FORMAT(inicio, '%H:%i') AS inicio, TIME_FORMAT(fim, '%H:%i') AS fim, modelo, solicitante, motorista, para_onde, observacao FROM agenda_veiculos, veiculos WHERE id_veiculo = veiculos.id AND agenda_veiculos.id = $id ORDER BY dia ASC");
	}	
}